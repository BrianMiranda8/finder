...
function folder_view($target)
{
    ...
    $item = scandir($_SERVER['DOCUMENT_ROOT'] . $target);
    // echo "<table>";
    foreach ($item as $i) {

        if ($i[0] != '.') {

            if (is_dir($_SERVER['DOCUMENT_ROOT'] . $target . '/' . $i)) {
                echo '<tr data-parent="' . $target . '" data-location="' . $target . "/" . $i . '" draggable="true">
                <td>
                <div class="stuff-items" data-type="dir">
            ...
            elseif (stripos($i, '.pdf') == true) {
                echo '<tr data-parent="' . $target . '" draggable="true"><td><div class="stuff-items"><div class="arrow"></div><div class="narrow" >';
